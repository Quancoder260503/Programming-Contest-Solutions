...
private:
    void build(vector<T> v)
    {
        ...
        for (int h = height - 1; h >= 0; --h)
        {
            ...
            data[h].do_build();
            const int m = stable_partition(v.begin(), v.end(), [&b](T const &x)
                                           { return x < b; }) -
                          v.begin();
            for (int i = m; i < n; ++i)
            {
